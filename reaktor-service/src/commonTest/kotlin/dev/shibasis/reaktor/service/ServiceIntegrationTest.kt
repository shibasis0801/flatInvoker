...
class ServiceIntegrationTest {
    ...
    fun serverInterceptorsWrapHandlerWithoutChangingDsl() = runTest {
        ...
        assertEquals(listOf(InterceptorStage.SERVER_APPLICATION), interceptor.stagesSeen)
        assertEquals("hello", response.message)
        assertEquals("true", response.headers["X-Intercepted"])
    }

    @Test
    fun stageSpecificInterceptorsOnlyRunForMatchingStage() = runTest {
        val skipped = RecordingInterceptor()
        val applied = RecordingInterceptor()
        val service = object : Service() {
            ...
            init {
                use(setOf(InterceptorStage.CLIENT_APPLICATION), skipped)
                use(setOf(InterceptorStage.SERVER_APPLICATION), applied)
            }
        }

        val response = service.ping(PingRequest(message = "stage"))

        assertEquals("stage", response.message)
        assertTrue(skipped.phases.isEmpty())
        assertEquals(listOf(InterceptorStage.SERVER_APPLICATION), applied.stagesSeen)
    }
